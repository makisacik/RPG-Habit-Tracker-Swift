...
class InventoryManager: ObservableObject {
    ...
    private init() {
        ...
        // Load initial inventory and effects
        refreshInventory()
    ...
    @objc private func handleLanguageChanged() {
        // Refresh inventory to update localized names
    ...
    func getGearItems(of category: GearCategory) -> [Item] {
        return ItemDatabase.getItems(of: category)
    }
    ...
    func getGearItems(of category: GearCategory, rarity: ItemRarity) -> [Item] {
        return ItemDatabase.getItems(of: category).filter { $0.rarity == rarity }
    }
